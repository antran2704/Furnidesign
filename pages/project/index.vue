    ...
    <div>
        ...
        <CategoryLayout :title="title" :description="description">
            <div class="grid md:grid-cols-2 grid-cols-1 gap-5">
                ...
                <div v-else v-for="product in projects" :key="product.id"
                    ...
                    <NuxtLink :to="`/project/${product.link}`">
                        <div class="h-64 rounded w-full object-cover object-center mb-6">
                            <Image classProps="w-full h-full" :src="product.picture" :alt="product.name" />
                        </div>
            ...
            <Pagination v-if="store.listItemsFilter.length > 16" :totalItems="store.listItemsFilter.length || 0"
                :currentPage="Number(route.params.page) || 1" :pageSize="16" url="/project/page" />
        </CategoryLayout>
    </div>
</template>

<script setup>
const title = ref('Design & Project')
...
const projects = computed(() => {
    var projectList = store.listItemsFilter;
    if (category) {
        projectList = projectList.filter(obj => obj.rooms.some(room => room.link === category));
    }
    return projectList.slice((route.params.page ? Number(route.params.page) - 1 : 0) * 16, (route.params.page ? Number(route.params.page) : 1) * 16)
});
...
useSeoMeta({
    title: 'Furni Design Project',
    ogTitle: 'Furni Design Project',
