<template>
    <div class="container px-5 mx-auto">
        <div class="pt-[150px]">
            <h2 class="md:text-4xl sm:text-3xl text-2xl font-cormorantGaramond font-bold text-center mb-4">{{title}}</h2>
            <p class="text-base text-[#44484c]">
                {{ description }}
            </p>
        </div>
                ...
                <div class="flex flex-col items-start justify-between gap-4">
                    <div  v-show="false" class="w-full">
                        ...
                        <ul class="flex flex-col px-3 gap-2">
                            <li v-for="category in listCategoriesRoom"
                            :key="category.Id"
                                class="flex items-center text-lg text-[#25282b] cursor-pointer gap-2">
                                <input @change="(e) => handleChangeFilter(e)"
                                    :checked="rooms.includes(category.name) ? true : false" type="checkbox" name="rooms"
                                    :value="category.name">
                                {{ category.name }}
                            </li>
                        </ul>
                    ...
                    <div  class="w-full">
                        ...
                        <ul class="flex flex-col px-3 gap-2">
                            <li v-for="category in listCategoriesStyle"
                            :key="category.id"
                                class="flex items-center text-lg text-[#25282b] cursor-pointer gap-2">
                                <input @change="(e) => handleChangeFilter(e)"
                                    :checked="styles.includes(category.name) ? true : false" type="checkbox" name="styles"
                                    :value="category.name">
                                {{ category.name }}
                            </li>
                        </ul>
                    ...
                    <div class="w-full">
                        ...
                        <ul class="flex flex-col px-3 gap-2">
                            <li v-for="category in listBudgets"
                              :key="category.id"
                                class="flex items-center text-lg text-[#25282b] cursor-pointer gap-2">
                                <input @change="(e) => handleChangeFilter(e)" type="checkbox"
                                    :checked="budget.includes(String(category.value)) ? true : false" :value="category.value"
                                    name="budget">
                                {{ category.name }}
                            </li>
                        </ul>
                    ...
                    <div class="w-full">
                        ...
                        <ul class="flex flex-col px-3 gap-2">
                            ...
                            :key="category.id"
                                class="flex items-center text-lg text-[#25282b] cursor-pointer gap-2">
                                <input @change="(e) => handleChangeFilter(e)" type="checkbox"
                                    :checked="rooms.includes(category.name) ? true : false" name="rooms"
                                    :value="category.name">
                                {{ category.name }}
                            </li>
                        </ul>
                    ...
                    <div class="w-full">
                        ...
                        <ul class="flex flex-col px-3 gap-2">
                            ...
                            :key="category.id"
                                class="flex items-center text-lg text-[#25282b] cursor-pointer gap-2">
                                <input @change="(e) => handleChangeFilter(e)" type="checkbox"
                                    :checked="styles.includes(category.name) ? true : false" :name="category.name">
                                {{ category.name }}
                            </li>
                        </ul>
                    </div>
...
const { rooms, budget, styles } = store.filters;

const isShowSelect = ref(false);
const listCategoriesRoom = await $fetch(`${config.public.apiEndpoint}/project/projectRoom`, {cache: "force-cache"});
const listCategoriesStyle = await $fetch(`${config.public.apiEndpoint}/project/projectStyle`, {cache: "force-cache"});
